...
Object(_utils_ios_vh_fix__WEBPACK_IMPORTED_MODULE_1__["iosVhFix"])(); // my dumb code
// ---------------------------------
...
var preloaderLogo = document.querySelector('.preloader .img-wrap');
var startButton = document.querySelector('.preloader .start-button');
var globalSiteWrapper = document.querySelector('.wrapper');
var bodyHtml = document.querySelector('html'); // bodyHtml.style.cssText = `padding-right: ${scrollBarWidth}px`;

var fadeInCss = function fadeInCss(duration) {
  return "transition: visibility ease ".concat(duration, ", opacity ease ").concat(duration, ";opacity: 1;visibility:visible;");
};

var fadeOutCss = function fadeOutCss(duration) {
  return "transition: visibility ease ".concat(duration, ", opacity ease ").concat(duration, ";opacity: 0;visibility:hidden;");
};

var globalSiteWrapperToggle = function globalSiteWrapperToggle() {
  globalSiteWrapper.classList.toggle('wrapper--no-scrollbar');
};

var onLoadEmulator = function onLoadEmulator(fetchingDelay) {
...
onLoadEmulator(1000).then(function () {
  preloaderLogo.style.cssText = fadeInCss('3s');
});
onLoadEmulator(4000).then(function () {
...
var closePreloader = function closePreloader() {
  var duration = 1;
  preloader.style.cssText = "transition: opacity ease ".concat(duration, "s;opacity:0;");
  setTimeout(function () {
    preloader.style.cssText += "display:none;";
    globalSiteWrapperToggle();
    globalSiteWrapper.style.cssText = "transition: opacity ease 1.2s;opacity: 1;"; // bodyHtml.style.cssText = `padding-right: 0`;
...
var burgerMenu = document.querySelector('.nav__list');
var burgerMenuButton = document.querySelector('.button-burger-mobile');
var burgerMenuLinks = document.querySelectorAll('.nav__link');
var headerTopLine = document.querySelector('.header__top-line');

var burgerMenuHandler = function burgerMenuHandler(duration) {
  ...
  if (!burgerMenu.classList.contains('nav__list-mobile_active')) {
    ...
    var paddingRight = scrollBarWidth + +/\d+/.exec(window.getComputedStyle(headerTopLine).paddingRight); // headerTopLine.style.cssText = `padding-right: ${paddingRight}px`;

    bodyHtml.classList.toggle('scroll-lock');
    burgerMenu.classList.toggle('nav__list-mobile_active');
    setTimeout(function () {
      burgerMenu.style.cssText = fadeInCss("".concat(duration, "s"));
    }, 0);
  } else {
    burgerMenu.style.cssText = fadeOutCss("".concat(duration, "s"));
    setTimeout(function () {
      burgerMenu.classList.toggle('nav__list-mobile_active');
      bodyHtml.classList.toggle('scroll-lock'); // bodyHtml.style.cssText = `padding-right: 0`;
...
var burgerMenuCleaner = function burgerMenuCleaner() {
  ...
  bodyHtml.classList.remove('scroll-lock'); // bodyHtml.style.cssText = `padding-right: 0`;
...
burgerMenuLinks.forEach(function (el) {
  el.addEventListener('click', function (e) {
    e.preventDefault(); // "scroll to" some code
...
var dataAttrButtonText = startButton.dataset.textMd;
var buttonText = startButton.innerHTML;
